                             ...
                             vector<cv::Point3f> &vP3D, vector<bool> &vbTriangulated)
{
    ...
    std::cout<<"HSocre_best:"<<SH<<std::endl;
    std::cout<<"H_best"<<cv::format(H,cv::Formatter::FMT_C)<<std::endl;


    //return ReconstructH(vbMatchesInliersH,H,mK,R21,t21,vP3D,vbTriangulated,1.0,50);
    cv::Mat n1i;
    vector <cv::Point2f> vPn1i;
    return ReconstructH(H, mK, R21, t21, n1i,vPn1i);
...
bool Initializer::ReconstructH(cv::Mat &H21, cv::Mat &K, cv::Mat &R21, cv::Mat &t21, cv::Mat &n1,vector <cv::Point2f> vPn1i) {
...
*/
            ...
            //条件二From Faguras
            //ntm1/d>0
            int nGood = CheckRT(mvMatches12, vR[i], vn[i],vd[i]);
            //cout << "nGood[" << i << "]:" << nGood <<"/"<<mvMatches12.size()<< endl;
...
bool Initializer::CheckVisibility(cv::Mat &H21,vector<cv::Point2f> vPn1i,float di){
...
int Initializer::CheckRT(vector <Match> vMatches12, cv::Mat R, cv::Mat N,float d) {
        int nGood = 0;
        for (size_t i = 0, iend = vMatches12.size(); i < iend; i++) {
            //条件二：
            //For all the reference points being visible, they must be in front of the camera.
            ...
            cv::Point pt1 = mvKeys1[mvMatches12[i].first].pt;
            cv::Point pt2 = mvKeys2[mvMatches12[i].second].pt;
            cv::Mat m(3, 1, CV_32F);
